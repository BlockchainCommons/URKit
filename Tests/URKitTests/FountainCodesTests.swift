...
struct FountainCodesTests {
    @Test func testCRC32() {
        let string = "Wolf"
        let checksum = CRC32.checksum(string.utf8Data)
        #expect(checksum == 0x598c84dc)
        #expect(checksum.serialized.hex == "598c84dc")
    }
    
    @Test func testCRC32_2() {
        ...
        #expect(checksum == 0x2f19f3bb)
        #expect(checksum.serialized.hex == "2f19f3bb")
    }
    ...
    @Test func testFindFragmentLength() {
        #expect(FountainEncoder.findNominalFragmentLength(messageLen: 12_345, minFragmentLen: 1_005, maxFragmentLen: 1_955) == 1_764)
        #expect(FountainEncoder.findNominalFragmentLength(messageLen: 12_345, minFragmentLen: 1_005, maxFragmentLen: 30_000) == 12_345)
    }

    @Test func testRandomSampler() {
        ...
        #expect(samples == expectedSamples)
        ...
        #expect(sortedValues == [28, 68, 130, 274])
        #expect(sortedValues.reduce(0, +) == samplesCount)
    }
    ...
    @Test func testPartitionAndJoin() {
        ...
        #expect(fragmentsHex == expectedFragments)
        let rejoinedMessage = FountainDecoder.joinFragments(fragments, messageLen: message.count)
        #expect(message == rejoinedMessage)
    }

    @Test func testDegreeChooser() {
        let message = makeMessage(len: 1024)
        let fragmentLen = FountainEncoder.findNominalFragmentLength(messageLen: message.count, minFragmentLen: 10, maxFragmentLen: 100)
        let fragments = FountainEncoder.partitionMessage(message, fragmentLen: fragmentLen)
        ...
        #expect(degrees == expectedDegrees)
        var totals: [Int: Int] = [:]
        ...
        let sortedDegrees = Array(totals)
            .sorted { $0.key < $1.key }
            .map { $0.value }
        #expect(sortedDegrees == [328, 151, 116, 77, 71, 54, 52, 55, 33, 33, 30])
        // Nominal values for 1000 samples: [331, 166, 110, 83, 66, 55, 47, 41, 37, 33, 30]
    }

    @Test func testFragmentChooser() {
        let message = makeMessage(len: 1024)
        ...
        #expect(fragmentIndexes == expectedFragmentIndexes)
    }

    @Test func testXOR() {
        let rng = Xoshiro256(string: "Wolf")
        let data1 = rng.nextData(count: 10)
        #expect(data1.hex == "916ec65cf77cadf55cd7")
        let data2 = rng.nextData(count: 10)
        #expect(data2.hex == "f9cda1a1030026ddd42e")
        var data3 = data1
        data2.xor(into: &data3)
        #expect(data3.hex == "68a367fdf47c8b2888f9")
        data1.xor(into: &data3)
        #expect(data3 == data2)
    }

    @Test func testEncoder() {
    ...
    @Test func testEncoderIsComplete() {
        let message = makeMessage(len: 256)
        let encoder = FountainEncoder(message: message, maxFragmentLen: 30)
        ...
        #expect(encoder.seqLen == generatedPartsCount)
    }

    @Test func testDecoder() {
    ...
    @Test func testCBOR() throws {
        ...
        let cbor = part.cbor
        #expect(cbor.hex == "850c0818641a12345678450105030305")
        #expect(try! CBOR(cbor).diagnostic() == """
        [
           12,
           8,
           100,
